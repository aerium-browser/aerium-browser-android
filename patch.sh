...
sed_i 's|if (!context->IsOffTheRecord()) {|if (!context->IsOffTheRecord() \|\| (base::FeatureList::GetInstance() \&\& base::FeatureList::GetInstance()->IsFeatureOverriddenFromCommandLine("AndroidOpenIncognitoAsWindow") \&\& !base::FeatureList::IsEnabled(extensions_features::kAeriumIncognitoWindowSharedExtensions))) {|' extensions/browser/process_manager.cc
