...
sed_i 's%^#endif  // CHROME_BROWSER_AERIUM_FLAG_ENTRIES_H_$%    {"aerium-incognito-window-shared-extensions",\n     "Share extensions with Incognito windows",\n     "When Seamless Incognito is off, Incognito opens as its own window and "\n     "Aerium gives it a separate copy of every extension. That copy never sees "\n     "your Incognito tabs, so popups like uBlock Origin say no access is "\n     "needed even while ads are blocked. Turn this on to let Incognito windows "\n     "use the normal extension instead, the way Chromium does. Experimental: "\n     "if extensions stop working in Incognito windows, turn it back off. "\n     "Aerium flag.",\n     kOsAll,\n     FEATURE_VALUE_TYPE(\n         extensions_features::kAeriumIncognitoWindowSharedExtensions)},\n&%' \
    chrome/browser/aerium_flag_entries.h
...
#
# Reuses a live window of the target type instead of stacking a new one every
# tap. MultiWindowUtils and PersistedInstanceType are both already imported in
# this file - nothing new to pull in. getUsableInstanceIds(ACTIVE | target)
# asks only for instances with a live task, of the profile type being
# switched to; launchIntentInInstance() is the same call Chrome's own
# notification and link-handling code uses to bring one of those to the
# front rather than starting a second copy. Only on an empty list, or every
# candidate failing to launch (the activity died between the query and the
# tap), does this fall back to startActivity() and open a new one - the
# original behaviour, not a broken one.
...
# false. None of that had a way to be reached; this pref is that way.
#
