...
# straight back.
sed_i 's|^</resources>$|    <!-- Aerium: see theme.sh. Pure black for OLED panels. -->\n    <style name="ThemeOverlay.BrowserUI.AeriumPureBlack" parent="">\n        <item name="android:colorBackground">@android:color/black</item>\n        <item name="colorSurface">@android:color/black</item>\n        <item name="colorSurfaceDim">@android:color/black</item>\n        <item name="colorSurfaceContainerLowest">@android:color/black</item>\n        <item name="colorSurfaceContainerLow">@android:color/black</item>\n        <item name="colorSurfaceContainer">@android:color/black</item>\n        <item name="colorSurfaceContainerHigh">@android:color/black</item>\n        <item name="colorSurfaceBright">@android:color/black</item>\n        <item name="colorSurfaceContainerHighest">@android:color/black</item>\n        <item name="aeriumIncognitoBgColor">@android:color/black</item>\n        <item name="elevationOverlayEnabled">false</item>\n        <!-- Aerium: pin the text roles too. A dialog inflated against a\n             context this overlay never reached (its own themed window,\n             rather than the Activity'"'"'s) otherwise keeps whatever on-surface\n             tone that context started with, which can end up too close to\n             this overlay'"'"'s black - the permission prompt being the case that\n             was actually seen. Plain white/near-white always reads against\n             black, so pinning it here can only help, never regress contrast\n             on a context this overlay does reach. -->\n        <item name="colorOnSurface">@android:color/white</item>\n        <item name="colorOnSurfaceVariant">#CCFFFFFF</item>\n        <item name="colorOnBackground">@android:color/white</item>\n        <item name="globalFilledButtonBgColor">?attr/colorPrimary</item>\n        <item name="globalFilledButtonTextColor">?attr/colorOnPrimary</item>\n        <item name="globalFilledTonalButtonBgColor">?attr/colorPrimaryContainer</item>\n        <item name="globalFilledTonalButtonTextColor">?attr/colorOnPrimaryContainer</item>\n        <item name="globalTextButtonTextColor">?attr/colorPrimary</item>\n    </style>\n&|' \
...
# Chromium 153 dropped this file's own use of CommandLine, so the import is
# gone from pristine upstream; added back here since the blocks below still
# need it.
sed_i 's|^import org.chromium.base.BinderCallsListener;$|&\nimport org.chromium.base.CommandLine;|' \
    $CAI
sed_i 's|^import org.chromium.chrome.browser.notifications.chime.ChimeDelegate;$|&\nimport org.chromium.chrome.browser.preferences.ChromePreferenceKeys;\nimport org.chromium.chrome.browser.preferences.ChromeSharedPreferences;|' \
    $CAI
...
sed_i 's|^                    sharedPreferencesManager.writeInt(UI_THEME_SETTING, theme);$|&\n                    showRestartSnackbar();|' \
    $TSF
...
sed_i 's|^import android.content.Context;$|import android.content.ActivityNotFoundException;\n&\nimport android.content.Intent;|' \
    $ACS
sed_i 's|^import org.chromium.chrome.browser.settings.ChromeBaseSettingsFragment;$|import org.chromium.chrome.browser.customtabs.CustomTabActivity;\n&|' \
    $ACS
sed_i 's|^    private static final String PREF_LEGAL_INFORMATION = "legal_information";$|&\n\n    // Aerium: see theme.sh.\n    private static final String PREF_AERIUM_PROJECT = "aerium_project";\n    private static final String AERIUM_PROJECT_URL = "https://aerium-browser.github.io";\n\n    // Aerium: see theme.sh.\n    private static final String PREF_AERIUM_SHARE = "aerium_share";|' \
    $ACS
sed_i 's|^        p.setSummary(getString(R.string.legal_information_summary, currentYear));$|&\n\n        // Aerium: see theme.sh. Opened in a Custom Tab so a link to this\n        // build'"'"'s own project cannot be answered by a different browser.\n        Preference project = findPreference(PREF_AERIUM_PROJECT);\n        if (project != null) {\n            project.setOnPreferenceClickListener(\n                    preference -> {\n                        CustomTabActivity.showInfoPage(getActivity(), AERIUM_PROJECT_URL);\n                        return true;\n                    });\n        }\n\n        // Aerium: see theme.sh. An ordinary system share sheet, not a link that\n        // opens anywhere itself - whoever receives it picks how to read it.\n        Preference share = findPreference(PREF_AERIUM_SHARE);\n        if (share != null) {\n            share.setOnPreferenceClickListener(\n                    preference -> {\n                        Intent sendIntent = new Intent(Intent.ACTION_SEND);\n                        sendIntent.setType("text/plain");\n                        sendIntent.putExtra(\n                                Intent.EXTRA_TEXT, getString(R.string.aerium_share_text));\n                        Intent chooser =\n                                Intent.createChooser(\n                                        sendIntent, getString(R.string.aerium_share_title));\n                        try {\n                            startActivity(chooser);\n                        } catch (ActivityNotFoundException e) {\n                            // No app on the device can handle a plain-text share. Nothing to\n                            // fall back to - the row simply does nothing this time.\n                        }\n                        return true;\n                    });\n        }|' \
    $ACS

sed_i 's|^</PreferenceScreen>$|    <Preference\n        android:key="aerium_project"\n        android:title="@string/aerium_project_title"\n        android:summary="@string/aerium_project_summary" />\n    <Preference\n        android:key="aerium_share"\n        android:title="@string/aerium_share_row_title"\n        android:summary="@string/aerium_share_row_summary" />\n&|' \
    chrome/android/java/res/xml/about_chrome_preferences.xml

sed_i 's|^      <message name="IDS_AERIUM_PURE_BLACK_TITLE" desc=|      <message name="IDS_AERIUM_PROJECT_TITLE" desc="Title of the About-page row that opens the browser project'"'"'s own website.">\n        Aerium project\n      </message>\n      <message name="IDS_AERIUM_PROJECT_SUMMARY" desc="Summary under that row: the address it opens.">\n        aerium-browser.github.io\n      </message>\n      <message name="IDS_AERIUM_SHARE_ROW_TITLE" desc="Title of the About-page row that opens a share sheet for telling someone else about the browser.">\n        Share Aerium\n      </message>\n      <message name="IDS_AERIUM_SHARE_ROW_SUMMARY" desc="Summary under that row.">\n        Tell a friend\n      </message>\n      <message name="IDS_AERIUM_SHARE_TITLE" desc="Title of the system share sheet opened by the Share Aerium row.">\n        Share Aerium\n      </message>\n      <message name="IDS_AERIUM_SHARE_TEXT" desc="The message shared by the Share Aerium row - a pitch plus the download link, sent as plain text to whatever app the person picks from the share sheet.">\n        Aerium is a de-googled Chromium browser with real Chrome extensions on Android, Manifest V2 included. No telemetry, no bundled Google services. https://github.com/aerium-browser/aerium-browser-android/releases/latest\n      </message>\n&|' \
    chrome/browser/ui/android/strings/android_chrome_strings.grd
...
sed_i 's%^#include "chrome/browser/browsing_data/chrome_browsing_data_remover_constants.h"$%#include "chrome/browser/browser_process.h"\n&%' \
    chrome/browser/browsing_data/chrome_browsing_data_lifetime_manager.cc
sed_i 's%^#include <utility>$%&\n#include <vector>%' \
    chrome/browser/browsing_data/chrome_browsing_data_lifetime_manager.cc
...
sed_i 's|^    !BUILDFLAG(IS_DESKTOP_ANDROID)$|    true|' \
    third_party/blink/common/features.cc
PSI=third_party/blink/renderer/platform/scheduler/main_thread/page_scheduler_impl.cc
sed_i 's|^#include "base/check_op.h"$|&\n#include "base/command_line.h"|' $PSI
sed_i 's|^base::TimeDelta GetDelayForBackgroundTabFreezing() {$|bool IsAeriumExtensionProcess() {\n  static const bool is_extension_process =\n      base::CommandLine::ForCurrentProcess()->HasSwitch("extension-process");\n  return is_extension_process;\n}\n\n&|' $PSI
sed_i 's|^                   blink::features::kStopInBackground)) {$|                   blink::features::kStopInBackground) \&\&\n               !IsAeriumExtensionProcess()) {|' $PSI
sed_i 's|^#if BUILDFLAG(IS_ANDROID) && !BUILDFLAG(IS_DESKTOP_ANDROID)$|#if BUILDFLAG(IS_ANDROID)|' \
    content/public/common/content_features.cc

echo "[aerium] background tab freezing and quiet console applied"

sed_i 's|^BASE_FEATURE(kSimdutfBase64Encode, base::FEATURE_DISABLED_BY_DEFAULT);$|BASE_FEATURE(kSimdutfBase64Encode, base::FEATURE_ENABLED_BY_DEFAULT);|' \
    base/features.cc
sed_i 's|^BASE_FEATURE(kLibraryPrefetcherMadvise, FEATURE_DISABLED_BY_DEFAULT);$|BASE_FEATURE(kLibraryPrefetcherMadvise, FEATURE_ENABLED_BY_DEFAULT);|' \
    base/features.cc
sed_i '/^BASE_FEATURE(kShutdownPreNativeThreadPoolAfterStartup,$/,+1 s|^             FEATURE_DISABLED_BY_DEFAULT);$|             FEATURE_ENABLED_BY_DEFAULT);|' \
    base/features.cc
sed_i 's|^BASE_FEATURE(kUtfConversionAsciiFastPath, FEATURE_DISABLED_BY_DEFAULT);$|BASE_FEATURE(kUtfConversionAsciiFastPath, FEATURE_ENABLED_BY_DEFAULT);|' \
    base/features.cc
sed_i 's|^BASE_FEATURE(kReclaimPrepaintTilesWhenIdle, base::FEATURE_DISABLED_BY_DEFAULT);$|BASE_FEATURE(kReclaimPrepaintTilesWhenIdle, base::FEATURE_ENABLED_BY_DEFAULT);|' \
    cc/base/features.cc
sed_i 's|^BASE_FEATURE(kReclaimOldPrepaintTiles, base::FEATURE_DISABLED_BY_DEFAULT);$|BASE_FEATURE(kReclaimOldPrepaintTiles, base::FEATURE_ENABLED_BY_DEFAULT);|' \
    cc/base/features.cc
sed_i '/^BASE_FEATURE(kCanvas2DHibernationReleaseTransferMemory,$/,+1 s|^             base::FeatureState::FEATURE_DISABLED_BY_DEFAULT);$|             base::FeatureState::FEATURE_ENABLED_BY_DEFAULT);|' \
    third_party/blink/common/features.cc

echo "[aerium] startup, text and memory defaults applied"
...
sed_i 's%^#endif  // CHROME_BROWSER_AERIUM_FLAG_ENTRIES_H_$%    {"aerium-reduce-hardware-concurrency",\n     "Report two CPU cores",\n     "Tells websites this device has two CPU cores, so the real count cannot "\n     "be used to recognise you. Heavy web apps may run slower. Aerium Guard "\n     "Privacy and Security turn this on. Aerium flag.",\n     kOsAll,\n     FEATURE_VALUE_TYPE(blink::features::kAeriumReduceHardwareConcurrency)},\n&%' \
    chrome/browser/aerium_flag_entries.h
sed_i 's%^#endif  // CHROME_BROWSER_AERIUM_FLAG_ENTRIES_H_$%    {"aerium-background-tab-freezing",\n     "Freeze background tabs",\n     "Pauses a tab a few minutes after you leave it, so it stops using battery "\n     "and data in the background. Tabs playing sound and extensions are never "\n     "paused. Turn this off if a web app must keep running in a background "\n     "tab. Aerium flag.",\n     kOsAll,\n     FEATURE_VALUE_TYPE(blink::features::kStopInBackground)},\n&%' \
    chrome/browser/aerium_flag_entries.h
sed_i 's%^#endif  // CHROME_BROWSER_AERIUM_FLAG_ENTRIES_H_$%    {"aerium-incognito-window-shared-extensions",\n     "Share extensions with Incognito",\n     "Aerium gives Incognito its own copy of every extension, because without "\n     "it extensions stop working there. That copy never sees your Incognito "\n     "tabs, so popups like uBlock Origin say no access is needed even while "\n     "ads are blocked. Turn this on to use the normal extension in Incognito "\n     "instead, the way Chromium does. Experimental: if extensions stop "\n     "working in Incognito, turn it back off. Aerium flag.",\n     kOsAll,\n     FEATURE_VALUE_TYPE(\n         extensions_features::kAeriumIncognitoWindowSharedExtensions)},\n&%' \
    chrome/browser/aerium_flag_entries.h
...
perl -0777 -pi -e '
    s!  unsigned int hardware_concurrency =\n      NavigatorConcurrentHardware::hardwareConcurrency\(\);\n!  unsigned int hardware_concurrency =\n      base::FeatureList::IsEnabled(features::kAeriumReduceHardwareConcurrency)\n          ? 2\n          : NavigatorConcurrentHardware::hardwareConcurrency();\n!
...
sed_i 's%^                    ContextCompat.getColor(context, R.color.default_bg_color_dark);$%                    org.chromium.components.browser_ui.styles.ChromeColors.getPrimaryBackgroundColor(\n                            context, true);%' \
    $HUBDIR/java/src/org/chromium/chrome/browser/hub/HubColors.java
sed_i 's%^</resources>$%    <item type="id" name="aerium_switch_window_menu_id" />\n&%' \
    $HUBDIR/res/values/ids.xml
HTM=$HUBDIR/java/src/org/chromium/chrome/browser/hub/HubToolbarMediator.java
sed_i 's%^        mHairlineVisibilitySupplier =$%        aeriumAddWindowSwitchButton();\n&%' $HTM
sed_i 's%^    private void rebuildPaneSwitcherButtonData() {$%    private static final int AERIUM_WINDOW_SWITCH_PANE_ID = -1;\n\n    private void aeriumAddWindowSwitchButton() {\n        int regularIndex = findCachedPaneSwitcherIndex(PaneId.TAB_SWITCHER);\n        int incognitoIndex = findCachedPaneSwitcherIndex(PaneId.INCOGNITO_TAB_SWITCHER);\n        boolean hasRegular = regularIndex != INVALID_PANE_SWITCHER_INDEX;\n        boolean hasIncognito = incognitoIndex != INVALID_PANE_SWITCHER_INDEX;\n        if (hasRegular == hasIncognito) return;\n        DisplayButtonData buttonData =\n                hasRegular\n                        ? new org.chromium.chrome.browser.ui.actions.button.ResourceButtonData(\n                                R.string.accessibility_tab_switcher_incognito_stack,\n                                R.string.accessibility_tab_switcher_incognito_stack,\n                                R.drawable.ic_incognito_24dp)\n                        : new org.chromium.chrome.browser.ui.actions.button.ResourceButtonData(\n                                R.string.aerium_hub_normal_tabs,\n                                R.string.aerium_hub_normal_tabs,\n                                R.drawable.btn_tabswitcher_modern);\n        int insertIndex = hasRegular ? regularIndex + 1 : incognitoIndex;\n        mCachedPaneSwitcherButtonData.add(\n                insertIndex, new Pair<>(AERIUM_WINDOW_SWITCH_PANE_ID, buttonData));\n    }\n\n    private void aeriumSwitchWindow() {\n        android.app.Activity activity =\n                org.chromium.base.ContextUtils.activityFromContext(mContext);\n        if (activity\n                instanceof org.chromium.components.browser_ui.widget.MenuOrKeyboardActionController\n                        controller) {\n            controller.onMenuOrKeyboardAction(R.id.aerium_switch_window_menu_id, false);\n        }\n        org.chromium.base.task.PostTask.postTask(\n                org.chromium.base.task.TaskTraits.UI_DEFAULT, this::rebuildPaneSwitcherButtonData);\n    }\n\n&%' $HTM
sed_i 's%^            @PaneId int paneId, DisplayButtonData referenceButtonData) {$%&\n        if (paneId == AERIUM_WINDOW_SWITCH_PANE_ID) {\n            return new DelegateButtonData.Builder(referenceButtonData)\n                    .setOnPress(\n                            view -> {\n                                if (mIgnoreTabLayoutSelection) return;\n                                aeriumSwitchWindow();\n                            })\n                    .build();\n        }%' $HTM
sed_i 's%^        } else if (id == R.id.new_incognito_window_menu_id) {$%        } else if (id == R.id.aerium_switch_window_menu_id) {\n            boolean toIncognito =\n                    !(mActivity instanceof ChromeTabbedActivity current\n                            \&\& current.isIncognitoWindow());\n            int currentWindowId =\n                    mActivity instanceof ChromeTabbedActivity tabbed\n                            ? tabbed.getWindowId()\n                            : INVALID_WINDOW_ID;\n            int targetWindowId =\n                    MultiWindowUtils.getLastAccessedWindowIdExcludingSelf(\n                            currentWindowId,\n                            PersistedInstanceType.ACTIVE\n                                    \| (toIncognito\n                                            ? PersistedInstanceType.OFF_THE_RECORD\n                                            : PersistedInstanceType.REGULAR));\n            if (targetWindowId != INVALID_WINDOW_ID) {\n                openWindow(targetWindowId, NewWindowAppSource.MENU);\n                return true;\n            }\n            return handleMenuOrKeyboardAction(\n                    toIncognito ? R.id.new_incognito_window_menu_id : R.id.new_window_menu_id,\n                    fromMenu);\n&%' \
    chrome/android/java/src/org/chromium/chrome/browser/multiwindow/MultiInstanceManagerApi31.java
sed_i 's|      <message name="IDS_AERIUM_MEDIA_TITLE" desc=|      <message name="IDS_AERIUM_HUB_NORMAL_TABS" desc="Content description of the tab switcher button that returns from the Private window to the normal tabs window.">\n        Normal tabs\n      </message>\n&|' \
    chrome/browser/ui/android/strings/android_chrome_strings.grd
...
perl -0777 -pi -e '
    s{^        if \(ntpShowing\n                \|\| tabSwitcherShowing$}
     {        if (tabSwitcherShowing}m
        or die "[aerium] FATAL: the controls-position condition in "
               . "ToolbarPositionController.calculateStateTransition() no longer "
...
sed_i 's|        setBackgroundColor(ChromeSemanticColorUtils.getHomeSurfaceBackgroundColor(getContext()));|&\n\n        if (!org.chromium.chrome.browser.toolbar.settings.AddressBarPreference\n                .isToolbarConfiguredToShowOnTop()) {\n            setGravity(android.view.Gravity.BOTTOM \| android.view.Gravity.CENTER_HORIZONTAL);\n            setPadding(\n                    getPaddingLeft(),\n                    getPaddingTop(),\n                    getPaddingRight(),\n                    getPaddingBottom()\n                            + getResources()\n                                    .getDimensionPixelSize(R.dimen.toolbar_height_no_shadow));\n        }|' \
    chrome/android/java/src/org/chromium/chrome/browser/ntp/NewTabPageLayout.java
sed_i 's|^    private @Nullable View mSearchBoxView;$|&\n    private boolean mAeriumFillsList;|' \
    chrome/android/java/src/org/chromium/chrome/browser/ntp/NewTabPageLayout.java
sed_i 's|^        super.onAttachedToWindow();$|&\n        if (!mAeriumFillsList\n                \&\& !org.chromium.chrome.browser.toolbar.settings.AddressBarPreference\n                        .isToolbarConfiguredToShowOnTop()) {\n            android.view.ViewParent ancestor = getParent();\n            while (ancestor instanceof View) {\n                ViewGroup.LayoutParams params = ((View) ancestor).getLayoutParams();\n                if (params == null \|\| params.height == ViewGroup.LayoutParams.MATCH_PARENT) {\n                    break;\n                }\n                ancestor = ancestor.getParent();\n            }\n            if (ancestor instanceof View) {\n                View list = (View) ancestor;\n                mAeriumFillsList = true;\n                setMinimumHeight(list.getHeight());\n                list.addOnLayoutChangeListener(\n                        (v, left, top, right, bottom, oldLeft, oldTop, oldRight, oldBottom) -> {\n                            if (getMinimumHeight() != bottom - top) {\n                                setMinimumHeight(bottom - top);\n                            }\n                        });\n            }\n        }|' \
    chrome/android/java/src/org/chromium/chrome/browser/ntp/NewTabPageLayout.java

sed_i 's|^        DefaultBrowserPromoUtils.getInstance().maybeRecordDeepLinkOutcome();$|&\n\n        String aeriumFeatures = CommandLine.getInstance().getSwitchValue("enable-features");\n        boolean aeriumBottomBarOn =\n                aeriumFeatures != null \&\& aeriumFeatures.contains("AndroidBottomBar");\n        if (aeriumBottomBarOn\n                == org.chromium.chrome.browser.toolbar.settings.AddressBarPreference\n                        .isToolbarConfiguredToShowOnTop()) {\n            getSnackbarManager()\n                    .showSnackbar(\n                            org.chromium.chrome.browser.ui.messages.snackbar.Snackbar.make(\n                                            getString(R.string.aerium_restart_to_apply),\n                                            new org.chromium.chrome.browser.ui.messages.snackbar\n                                                    .SnackbarManager.SnackbarController() {\n                                                @Override\n                                                public void onAction(\n                                                        @Nullable Object actionData) {\n                                                    org.chromium.chrome.browser.lifetime\n                                                            .ApplicationLifetime.terminate(true);\n                                                }\n                                            },\n                                            org.chromium.chrome.browser.ui.messages.snackbar\n                                                    .Snackbar.TYPE_ACTION,\n                                            org.chromium.chrome.browser.ui.messages.snackbar\n                                                    .Snackbar.UMA_UNKNOWN)\n                                    .setAction(getString(R.string.aerium_relaunch), null)\n                                    .setDuration(10000));\n        }|' \
    chrome/android/java/src/org/chromium/chrome/browser/ChromeTabbedActivity.java
...
# this build is Vanadium and inherits none of them. Issue 15 is what that costs
# in practice - a reader following the README looked for a dozen flags and found
# the one that came from upstream Chromium rather than from ungoogled.
#
# These four are the ones that were named as missing and that mean something on
# a phone. Same flag names as the desktop builds, deliberately: one name per
# behaviour across all three platforms is the point of doing this at all.
#
# Not ported, and not an oversight: popups-to-tabs. Chrome on Android has no
# popup windows - window.open with features already lands in a tab - so the
# switch would toggle nothing. The rest of ungoogled's flag set is either
# desktop-only UI (tab strip, avatar button, hover cards, grab handle) or a
# later batch.
#
# Two of the four are command-line switches rather than features. That works
# here because both reads happen in the BROWSER process: about_flags puts the
# switch on the browser command line at startup, and RenderProcessHostImpl only
# forwards an allowlist to renderers, so a renderer-side read of a custom switch
# would silently never fire. ungoogled's own renderer-side halves of these two
# patches are left out for that reason rather than copied and left dead.

# --- disable-search-engine-collection.
#
# Chromium adds a search engine for any site that ships an OpenSearch document
# and gets typed into the omnibox, which quietly turns browsing history into a
# list in Settings. ungoogled gates that on a switch.
#
# Their patch has three parts. Only this one is kept: the two entry points that
# decide whether a keyword may be autogenerated both live in TemplateURLService,
# in the browser process, and closing them closes the behaviour whatever the
# renderer offered. The renderer half (chrome_render_frame_observer.cc) could
# not read the switch here anyway, and the third part edits
# chrome/browser/ui/tab_helpers.cc, which is the desktop tab helper list -
# Android attaches its own.
#
# Read live rather than cached in a member, which is what ungoogled does. The
# command line does not change after startup, HasSwitch is a small map lookup,
# and skipping the member means skipping the header change and the constructor
...
#
# Show an internationalized domain as its punycode rather than its Unicode, so
# a homograph cannot wear another site'"'"'s name. Chromium has spoof checks for
# this; the flag is for anyone who would rather not rely on them. Off by
# default, because turning it on makes every legitimate non-Latin domain
# unreadable, which is a real cost to most of the world.
#
# The early return goes after host16 is filled and before the spoof-check
# machinery, which is where ungoogled puts it: the punycode is already in
# host16 at that point, so returning it is returning the input unconverted.
UF=components/url_formatter/url_formatter.cc
sed_i 's%^#include "base/memory/raw_ptr.h"$%#include "base/command_line.h"\n&%' $UF
sed_i 's%^  host16.insert(host16.end(), host.begin(), host.end());$%&\n\n  // Aerium: see theme.sh. Leave the name as punycode.\n  if (base::CommandLine::ForCurrentProcess()->HasSwitch(\n          "force-punycode-hostnames")) {\n    IDNConversionResult aerium_result;\n    aerium_result.result = host16;\n    return aerium_result;\n  }%' $UF

# --- increase-incognito-storage-quota.
#
# Incognito computes its storage quota from physical memory by a different
# algorithm to a normal profile, and the difference is measurable from
# JavaScript, which is one of the ways a site detects incognito. This makes
# incognito use the regular algorithm, with physical memory standing in for disk
...
sed_i 's%^#include "base/functional/bind.h"$%#include "base/feature_list.h"\n&%' $QS
sed_i 's%^  if (is_incognito) {$%  // Aerium: see theme.sh.\n  if (is_incognito \&\&\n      !base::FeatureList::IsEnabled(features::kIncreaseIncognitoStorageQuota)) {%' $QS
sed_i 's%^  int64_t total = device_info_helper->AmountOfTotalDiskSpace(partition_path);$%  // Aerium: see theme.sh. Incognito has no disk of its own to measure.\n  int64_t total =\n      is_incognito ? device_info_helper->AmountOfPhysicalMemory()\n                   : device_info_helper->AmountOfTotalDiskSpace(partition_path);%' $QS

# --- remove-client-hints.
#
# Client hints are the structured half of the user agent: the headers a site
# gets without asking, plus navigator.userAgentData for the ones it does ask
# for. Vanadium already derives the high-entropy hints from the reduced user
# agent, which narrows them; this stops them being sent at all.
#
# A feature rather than a switch, so it reaches renderers on its own through
# --enable-features - which is what the last two of these five need.
sed_i 's%^const base::FeatureParam<std::string> kAeriumTimeZoneIdParam{&kAeriumTimeZone, kAeriumTimeZoneId, "random"};$%&\nBASE_FEATURE(kRemoveClientHints, "RemoveClientHints", base::FEATURE_DISABLED_BY_DEFAULT);%' \
    third_party/blink/common/features.cc
sed_i 's%^BLINK_COMMON_EXPORT extern const base::FeatureParam<std::string> kAeriumTimeZoneIdParam;$%&\nBLINK_COMMON_EXPORT BASE_DECLARE_FEATURE(kRemoveClientHints);%' \
    third_party/blink/public/common/features.h
sed_i 's%^BASE_FEATURE(kRemoveClientHints, "RemoveClientHints", base::FEATURE_DISABLED_BY_DEFAULT);$%&\nBASE_FEATURE(kAeriumReduceHardwareConcurrency, base::FEATURE_DISABLED_BY_DEFAULT);%' \
    third_party/blink/common/features.cc
sed_i 's%^BLINK_COMMON_EXPORT BASE_DECLARE_FEATURE(kRemoveClientHints);$%&\nBLINK_COMMON_EXPORT BASE_DECLARE_FEATURE(kAeriumReduceHardwareConcurrency);%' \
    third_party/blink/public/common/features.h

perl -0777 -pi -e '
    s!(void UpdateNavigationRequestClientUaHeaders\(\n    const url::Origin& origin,\n    ClientHintsControllerDelegate\* delegate,\n    bool override_ua,\n    FrameTreeNode\* frame_tree_node,\n    net::HttpRequestHeaders\* headers,\n    const std::optional<GURL>& request_url\) \{\n  DCHECK\(frame_tree_node\);\n)!$1  // Aerium: see theme.sh.\n  if (base::FeatureList::IsEnabled(blink::features::kRemoveClientHints)) {\n    return;\n  }\n!
        or die "[aerium] FATAL: UpdateNavigationRequestClientUaHeaders in "
             . "client_hints.cc no longer has the signature this expects\n";
    s!(    FrameTreeNode\* ftn_for_devtools_override\) \{\n)(  ClientHintsExtendedData data\(origin, frame_tree_node, delegate\);\n)!$1  // Aerium: see theme.sh.\n  if (base::FeatureList::IsEnabled(blink::features::kRemoveClientHints)) {\n    return;\n  }\n$2!
        or die "[aerium] FATAL: AddRequestClientHintsHeaders in client_hints.cc "
             . "no longer opens by building ClientHintsExtendedData\n";
' content/browser/client_hints/client_hints.cc

NUA=third_party/blink/renderer/core/frame/navigator_ua.cc
sed_i 's%^#include "base/compiler_specific.h"$%&\n#include "third_party/blink/public/common/features.h"%' $NUA
sed_i 's%^  UserAgentMetadata metadata = GetUserAgentMetadata();$%  // Aerium: see theme.sh. An empty NavigatorUAData, not a null one - the IDL\n  // says userAgentData never returns null.\n  if (base::FeatureList::IsEnabled(blink::features::kRemoveClientHints)) {\n    return ua_data;\n  }\n\n&%' $NUA

FFC=third_party/blink/renderer/core/loader/frame_fetch_context.cc
sed_i 's%^#include "third_party/blink/public/common/device_memory/approximated_device_memory.h"$%&\n#include "third_party/blink/public/common/features.h"%' $FFC
perl -0777 -pi -e '
    s!(void FrameFetchContext::AddClientHintsIfNecessary\(\n    const std::optional<float> resource_width,\n    ResourceRequest& request\) \{\n)!$1  // Aerium: see theme.sh.\n  if (base::FeatureList::IsEnabled(blink::features::kRemoveClientHints)) {\n    return;\n  }\n!
        or die "[aerium] FATAL: FrameFetchContext::AddClientHintsIfNecessary no "
             . "longer has the signature this expects\n";
' $FFC

# The four entries. Appended to the file the time zone block writes, so this
# has to run after it - it is at the end of theme.sh for that reason.
sed_i 's%^#endif  // CHROME_BROWSER_AERIUM_FLAG_ENTRIES_H_$%    {"disable-search-engine-collection",\n     "Disable search engine collection",\n     "Stop Aerium adding a search engine for every site that offers one. Sites "\n     "you have already collected stay in Settings until you remove them. "\n     "ungoogled-chromium flag, ported to Android by Aerium.",\n     kOsAll, SINGLE_VALUE_TYPE("disable-search-engine-collection")},\n    {"force-punycode-hostnames",\n     "Force punycode hostnames",\n     "Show an internationalized domain name as its punycode rather than its "\n     "own script, so a lookalike name cannot pass for another site. Costs "\n     "readability on every legitimate non-Latin domain. ungoogled-chromium "\n     "flag, ported to Android by Aerium.",\n     kOsAll, SINGLE_VALUE_TYPE("force-punycode-hostnames")},\n    {"increase-incognito-storage-quota",\n     "Increase incognito storage quota",\n     "Work out the incognito storage quota the way a normal profile does, "\n     "which is one of the numbers a site reads to tell whether you are in "\n     "incognito. Lets a page hold more in memory than incognito otherwise "\n     "allows. ungoogled-chromium flag, ported to Android by Aerium.",\n     kOsAll,\n     FEATURE_VALUE_TYPE(storage::features::kIncreaseIncognitoStorageQuota)},\n    {"remove-client-hints",\n     "Remove client hints",\n     "Stop sending client hints - the headers describing this device that go "\n     "out alongside the User-Agent - and hand navigator.userAgentData nothing "\n     "to report. Aerium already derives them from the reduced User-Agent; this "\n     "removes them. Some sites use them to pick a layout. "\n     "ungoogled-chromium flag, ported to Android by Aerium.",\n     kOsAll, FEATURE_VALUE_TYPE(blink::features::kRemoveClientHints)},\n&%' \
    chrome/browser/aerium_flag_entries.h
...
# Chromium 153 dropped chrome/browser's own dependency on
# network_session_configurator/browser, so that anchor is gone; the
# neighbouring network_time dep is still there and still alphabetically
# adjacent to where "common" belongs.
sed_i 's%^    "//components/network_time",$%    "//components/network_session_configurator/common",\n&%' \
...
# (mFeedSurfaceProvider.getView()), not NewTabPageScrollView, which upstream's own
...
PSJ=components/prefs/android/java/src/org/chromium/components/prefs/PrefService.java
sed_i 's|^import org.jni_zero.CalledByNative;$|&\nimport org.jni_zero.JniType;|' $PSJ
sed_i 's|^    public void setString(String preference, String value) {$|    public String[] getStringList(String preference) {\n        return PrefServiceJni.get().getStringList(mNativePrefServiceAndroid, preference);\n    }\n\n    public void setStringList(String preference, String[] value) {\n        PrefServiceJni.get().setStringList(mNativePrefServiceAndroid, preference, value);\n    }\n\n    public int getPrefType(String preference) {\n        return PrefServiceJni.get().getPrefType(mNativePrefServiceAndroid, preference);\n    }\n\n&|' $PSJ
sed_i 's|^        void setString(long nativePrefServiceAndroid, String preference, String value);$|&\n\n        @JniType("std::vector<std::string>")\n        String[] getStringList(long nativePrefServiceAndroid, String preference);\n\n        void setStringList(\n                long nativePrefServiceAndroid,\n                String preference,\n                @JniType("std::vector<std::string>") String[] value);\n\n        int getPrefType(long nativePrefServiceAndroid, String preference);|' $PSJ

PSH=components/prefs/android/pref_service_android.h
sed_i 's|^#include "base/android/jni_android.h"$|#include <string>\n#include <vector>\n\n&|' $PSH
sed_i 's|^                 const base::android::JavaRef<jstring>& j_value);$|&\n  std::vector<std::string> GetStringList(\n      JNIEnv* env,\n      const base::android::JavaRef<jstring>\& j_preference);\n  void SetStringList(JNIEnv* env,\n                     const base::android::JavaRef<jstring>\& j_preference,\n                     std::vector<std::string> value);\n  int32_t GetPrefType(JNIEnv* env,\n                      const base::android::JavaRef<jstring>\& j_preference);|' $PSH

PSC=components/prefs/android/pref_service_android.cc
sed_i 's|^bool PrefServiceAndroid::IsManagedPreference($|std::vector<std::string> PrefServiceAndroid::GetStringList(\n    JNIEnv* env,\n    const JavaRef<jstring>\& j_preference) {\n  std::vector<std::string> values;\n  for (const base::Value\& value : pref_service_->GetList(\n           base::android::ConvertJavaStringToUTF8(env, j_preference))) {\n    if (value.is_string()) {\n      values.push_back(value.GetString());\n    }\n  }\n  return values;\n}\n\nvoid PrefServiceAndroid::SetStringList(\n    JNIEnv* env,\n    const JavaRef<jstring>\& j_preference,\n    std::vector<std::string> value) {\n  base::ListValue list;\n  for (std::string\& entry : value) {\n    list.Append(std::move(entry));\n  }\n  pref_service_->SetList(\n      base::android::ConvertJavaStringToUTF8(env, j_preference),\n      std::move(list));\n}\n\nint32_t PrefServiceAndroid::GetPrefType(\n    JNIEnv* env,\n    const JavaRef<jstring>\& j_preference) {\n  const PrefService::Preference* preference = pref_service_->FindPreference(\n      base::android::ConvertJavaStringToUTF8(env, j_preference));\n  return preference ? static_cast<int32_t>(preference->GetType()) : -1;\n}\n\n&|' $PSC
...
public class AeriumBackupFragment extends ChromeBaseSettingsFragment {
    ...
    private static final int PREF_TYPE_DOUBLE = 3;
    ...
    private static final String[] STOCK_SHARED_PREFS = {
        ChromePreferenceKeys.HOMEPAGE_ENABLED,
        ChromePreferenceKeys.HOMEPAGE_USE_CHROME_NTP,
    ...
    private static final String[] PROFILE_PREFS = {
        "browser.clear_data.aerium_clear_on_exit",
        "browser.clear_data.aerium_ephemeral_all_sites",
        "browser.clear_data.aerium_ephemeral_clear_cache",
        "browser.clear_data.aerium_ephemeral_delay_seconds",
    ...
    private static final String[] LOCAL_STATE_PREFS = {
        FLAGS_PREF,
        "dns_over_https.mode",
        "dns_over_https.templates",
        Pref.IS_OMNIBOX_IN_BOTTOM_POSITION,
    };
    ...
    private void writeBackup(Uri uri) {
        ...
        byte[] bytes;
        try {
            bytes = buildBackup().toString(2).getBytes(StandardCharsets.UTF_8);
        } catch (JSONException e) {
    ...
    private @Nullable PrefService getProfilePrefs() {
        Profile profile = getProfile();
        return profile == null ? null : UserPrefs.get(profile);
    }
    ...
    private JSONObject buildBackup() throws JSONException {
        ...
        root.put("permission_defaults", collectPermissionDefaults());
        JSONObject settings = new JSONObject();
        settings.put("shared_preferences", collectSharedPreferences());
        settings.put("profile_prefs", collectPrefs(getProfilePrefs(), PROFILE_PREFS));
        settings.put("local_state_prefs", collectPrefs(LocalStatePrefs.get(), LOCAL_STATE_PREFS));
    ...
    private void applyBackup(JSONObject root) throws JSONException {
        ...
        if (settings != null) {
            ...
            if (profilePrefs != null) restorePrefs(getProfilePrefs(), PROFILE_PREFS, profilePrefs);

            JSONArray localStatePrefs = settings.optJSONArray("local_state_prefs");
            if (localStatePrefs != null) {
                restorePrefs(LocalStatePrefs.get(), LOCAL_STATE_PREFS, localStatePrefs);
            }
        }

        JSONArray permissionDefaults = root.optJSONArray("permission_defaults");
        if (permissionDefaults != null) restorePermissionDefaults(permissionDefaults);
    ...
    private JSONArray collectTabs() throws JSONException {
        ...
        for (TabModelSelector selector :
            ...
            for (int i = 0; i < model.getCount(); i++) {
                ...
                if (isInternalScheme(url.getScheme())) continue;
    ...
    private void restoreTabs(JSONArray tabs) {
        ...
        for (int i = 0; i < tabs.length(); i++) {
            ...
            if (isInternalScheme(parsedUrl.getScheme())) continue;
    ...
    private static boolean isBackedUpPermissionType(int type) {
        for (int candidate : PERMISSION_TYPES) {
    ...
    private void restorePermissions(JSONArray permissions) {
        ...
        for (int i = 0; i < permissions.length(); i++) {
            ...
            if (!isBackedUpPermissionType(type) || primary.isEmpty()) continue;
            if (primary.equals(WebsitePreferenceBridge.SITE_WILDCARD)) continue;
    ...
    private JSONArray collectPermissionDefaults() throws JSONException {
        JSONArray array = new JSONArray();
        Profile profile = getProfile();
        if (profile == null) return array;
        for (int type : PERMISSION_TYPES) {
            if (WebsitePreferenceBridge.isContentSettingManaged(profile, type)) continue;
            JSONObject entry = new JSONObject();
            entry.put("type", type);
            entry.put("setting", WebsitePreferenceBridge.getDefaultContentSetting(profile, type));
            array.put(entry);
        }
        return array;
    }

    private void restorePermissionDefaults(JSONArray defaults) {
        Profile profile = getProfile();
        if (profile == null) return;
        for (int i = 0; i < defaults.length(); i++) {
            JSONObject entry = defaults.optJSONObject(i);
            if (entry == null) continue;
            int type = entry.optInt("type", -1);
            int setting = entry.optInt("setting", -1);
            if (!isBackedUpPermissionType(type) || setting < 0) continue;
            if (WebsitePreferenceBridge.isContentSettingManaged(profile, type)) continue;
            if (WebsitePreferenceBridge.getDefaultContentSetting(profile, type) == setting) {
                continue;
            }
            try {
                WebsitePreferenceBridge.setDefaultContentSetting(profile, type, setting);
            } catch (RuntimeException e) {
            }
        }
    }

    private static boolean isBackedUpSharedPreference(String key) {
        return key.startsWith(SHARED_PREF_PREFIX) || Arrays.asList(STOCK_SHARED_PREFS).contains(key);
    }
    ...
    private JSONArray collectSharedPreferences() throws JSONException {
        ...
        for (Map.Entry<String, ?> entry : all.entrySet()) {
            ...
            if (!isBackedUpSharedPreference(key)) continue;
    ...
    private void restoreSharedPreferences(JSONArray items) {
        ...
        for (int i = 0; i < items.length(); i++) {
            ...
            if (key.isEmpty() || !isBackedUpSharedPreference(key)) continue;
    ...
    private static JSONArray collectPrefs(@Nullable PrefService prefs, String[] keys)
            throws JSONException {
        JSONArray array = new JSONArray();
        if (prefs == null) return array;
        for (String key : keys) {
            JSONObject item = new JSONObject();
            switch (prefs.getPrefType(key)) {
                ...
                case PREF_TYPE_DOUBLE:
                    ...
                    item.put("value", prefs.getDouble(key));
                    break;
                ...
                case PREF_TYPE_LIST:
                    ...
                    for (String value : prefs.getStringList(key)) {
                        values.put(value);
                    }
    ...
    private static void restorePrefs(
            @Nullable PrefService prefs, String[] keys, JSONArray items) {
        ...
        for (int i = 0; i < items.length(); i++) {
            ...
            int liveType = prefs.getPrefType(key);
            if (liveType < 0 || prefs.isManagedPreference(key)) continue;
            switch (item.optString("type", "")) {
                case "bool":
                    if (liveType == PREF_TYPE_BOOLEAN) {
                        prefs.setBoolean(key, item.optBoolean("value", false));
                    }
                    break;
                case "int":
                    if (liveType == PREF_TYPE_INTEGER) {
                        prefs.setInteger(key, item.optInt("value", 0));
                    }
                    break;
                case "double":
                    if (liveType == PREF_TYPE_DOUBLE) {
                        prefs.setDouble(key, item.optDouble("value", 0));
                    }
                    break;
                case "string":
                    if (liveType == PREF_TYPE_STRING) {
                        prefs.setString(key, item.optString("value", ""));
                    }
                    break;
                case "list":
                    JSONArray values = item.optJSONArray("value");
                    if (liveType == PREF_TYPE_LIST && values != null) {
                        String[] strings = new String[values.length()];
                        for (int j = 0; j < values.length(); j++) {
                            strings[j] = values.optString(j, "");
                        }
...
<PreferenceScreen xmlns:android="http://schemas.android.com/apk/res/android">
    ...
    <Preference
        android:key="aerium_guard_security"
        android:persistent="false"
